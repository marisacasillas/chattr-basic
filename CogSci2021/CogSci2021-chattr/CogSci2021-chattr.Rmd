...
We demonstrate the use of `chattr` with three child language environment datasets from unrelated rural Indigenous communities. <!-- it would be more specfici here: more precisely how it can be used as a sanity check type of system - to quickly check some basic distribution of your dataset, and also to go beyond and easily test theory driven hypotheses. --> These recordings document children's verbal interactional patterns over full days at home in underrepresented populations.<!-- maybe adding: for which automatic speech analysis doesnt exist --  or something to highlight that what is available hasnt been proven for nonweird languages -->  `Chattr` allows us to examine interactional patterns at scale in these corpora, evading months of manual annotation that would achieve the same <!--  I would also add : that it allows to inspect the data beyond idiosyncratic definitions of turns: in this case exchanges between children--> . The first two corpora, Tseltal (Mayan; Chiapas, Mexico; N = 10) and Yélî Dnye (isolate; Milne Bay, Papua New Guinea; N = 10), come from the Casillas HomeBank repository [@Casillas-HB] and were made with near parallel methods: children under age 3;0 wore an Olympus WS-832/853 audio recorder at home for 8--11 hours. The third corpus, Tsimane' (Tsimane'; Bolivia; N = 27) features children under 6;0 who wore one of multiple recording devices (LENA, Olympus, or USB) at home for 4--21 hours [@scaffIPlanguage]; we focus here on the subset of those 27 recordings made with LENA (N = 17). <!--  we still here need to define this without over complexifying, we have 40 different recordings from 27 different children; 17 recordings were made with LENA and had its files  (for one LENA recording we lost the upl and therefore the its)  of 13 different children --> In each dataset we assess the baseline turn-taking rate over age and the frequency of interactions with other children. For the Tsimane' corpus can also compare `chattr` estimates on both LENA (automated) and manually created annotations of the same recording minutes. This brief analysis tests whether previously documented patterns in these children's linguistic input are recapitulated in their turn-taking behavior, as detected by `chattr`.
<!-- Something that it is puzzling for me is the comparison between the baseline between the 3 hand annotated datasets, as the Tsimane one has a different definition because it uses the LENA one. I wonder how hard it would be to change the defnition to match the core one and see how much definition change the results? - if it is too much happy to drop it. but it is a bit hard for me to compare then the baseline -->
...
The mean rate of turn transitions in the Tseltal corpus was 3 and 11.8 transitions per minute for the random and active (turn taking and vocal activity) clips, respectfully, and 2.4 and 12.8 transitions per minute for Yélî Dnye. The distribution of turn taking rates across annotated clips was similar between the two sites (Table 1) <!-- small comment: I would align the figure and text here - in the figure we see on the left first active and then random - I think it is slighty easier to match the names and colors that way, but it is kind of overly picky so feel free to ignore, so in the text above it would mean to reverse the order  --> . A linear mixed effects regression of transitions per minute with predictors of clip type, corpus, and their interaction, and a random intercept for child reveals that random clips indeed have significantly lower transition rates (B = `r round(subset(model.outputs, model == "ttr_TSYD")$estimate[2],2)`, SE = `r round(subset(model.outputs, model == "ttr_TSYD")$std.error[2],2)`, t = `r round(subset(model.outputs, model == "ttr_TSYD")$statistic[2],2)`). There is no evidence for a significant difference in rates between languages (t = `r round(subset(model.outputs, model == "ttr_TSYD")$statistic[3],2)`) and no evidence for a clip type-language interaction (t = `r round(subset(model.outputs, model == "ttr_TSYD")$statistic[4],2)`).
