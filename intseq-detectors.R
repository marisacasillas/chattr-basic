...
fetch_intseqs <- function(tttbl, allowed.gap) {
  seq.stats <- tibble::tibble(
    speaker = character(),
    intseq.num = integer(),
    seq.start.ms = integer(),
    seq.stop.ms = integer()
  )
  n.poss.seqs <- tibble::tibble(
    intseq.num = c(1:nrow(tttbl))
  )
  tttbl.seq <- dplyr::left_join(tttbl, seq.stats, by = "speaker") %>%
    dplyr::select(-seq.start.ms, -seq.stop.ms)
  seq.stats <- dplyr::right_join(dplyr::select(seq.stats, -speaker), n.poss.seqs,
    by = "intseq.num")
  ...
  for (i in 2:nrow(tttbl.edges)) {
    tttbl.edges$focal.seq[i] <- dplyr::case_when(
  ...
  int.as.voc.seqs <- tttbl.edges %>%
    ...
    dplyr::filter(n.tts > 0 & !is.na(vocseq.num))
  ...
  if (nrow(tooclose.intseqs) > 0) {
    ...
    tooclose.intseqs <- tooclose.intseqs %>%
      dplyr::mutate(ms.since.prev.intseq = seq.start.ms - prev.seq.end) %>%
      dplyr::filter(ms.since.prev.intseq <= allowed.gap) %>%
      dplyr::pull(intseq.num)
  ...
  uniq.L.anchors.intseq <- tttbl.edges %>%
    dplyr::arrange(anchor.L.start.ms) %>%
    dplyr::group_by(intseq.num) %>%
    dplyr::mutate(id = dplyr::row_number()) %>%
    dplyr::filter(intseq.num > 0 & id == 1) %>%
  ...
  uniq.R.anchors.intseq <- tttbl.edges %>%
    dplyr::arrange(intseq.num, -anchor.R.stop.ms) %>%
    dplyr::group_by(intseq.num) %>%
    dplyr::mutate(id = dplyr::row_number()) %>%
    dplyr::filter(intseq.num > 0 & id == 1) %>%
  ...
  if (nrow(seq.stats.intseq) > 0) {
    seq.stats.intseq <- seq.stats.intseq %>%
      dplyr::summarize(
        ...
        seq.start.ms = min(anchor.L.start.ms),
        seq.stop.ms = max(anchor.R.stop.ms)) %>%
      dplyr::left_join(uniq.L.anchors.intseq, by = c("intseq.num", "seq.start.ms")) %>%
      dplyr::left_join(uniq.R.anchors.intseq, by = c("intseq.num", "seq.stop.ms")) %>%
      dplyr::rename(
    ...
    seq.stats.intseq <- tibble::tibble(
      intseq.num = double(),
  ...
  if (nrow(seq.stats.vocseq) > 0) {
    seq.stats.vocseq <- seq.stats.vocseq %>%
      dplyr::summarize(
        ...
        seq.start.ms = min(anchor.L.start.ms),
        seq.stop.ms = max(anchor.R.stop.ms)) %>%
      dplyr::left_join(uniq.L.anchors.vocseq, by = c("vocseq.num", "seq.start.ms")) %>%
      dplyr::left_join(uniq.R.anchors.vocseq, by = c("vocseq.num", "seq.stop.ms")) %>%
      dplyr::rename(
    ...
    seq.stats.vocseq <- tibble::tibble(
      vocseq.num = double(),
  ...
  intseqtbl <- tttbl.edges %>%
    ...
    dplyr::left_join(seq.stats.intseq, by = "intseq.num") %>%
    dplyr::left_join(seq.stats.vocseq, by = "vocseq.num") %>%
